...
Cada par dimensão/`subject` termina em `SPECIFIED`, `NOT_APPLICABLE`, `DECISION_REQUIRED` ou `GAP_FOUND`. O Harness, não a IA, calcula a autoridade de fechamento. Texto do modelo sozinho não pode certificar uma regra nem sua inaplicabilidade. `SPECIFIED` exige regra autorizada, os dois lados do witness em um caso exato e uma obrigação de prova sobre observáveis concretos; uma regra de invariância também é `SPECIFIED`. `NOT_APPLICABLE` é reservado à ausência estrutural autorizada da operação que ativaria a dimensão, nunca à simples afirmação de que duas saídas seriam iguais. `DECISION_REQUIRED` fica provisório até escolha humana. `GAP_FOUND` bloqueia a criação do Wizard. O witness descreve uma prova metamórfica futura, mas o preflight não executa nem cria produto ou testes. O contrato expõe ainda `effectiveDeterminismStatus`, recalculado depois das resoluções humanas: `SEMANTICALLY_CLOSED` significa que o contrato está semanticamente fechado, não que uma implementação já foi verificada.
...
- [ ] **Adaptador de IA Ponta a Ponta:** Um provedor externo consome `aegis.semantic_request.v8` e devolve somente `aegis.semantic_opinion.v1`.
