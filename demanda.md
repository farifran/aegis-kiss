...
Não são enviados à IA metadados da captura, contadores, tamanhos, hashes de arquivos, termos sem correspondência, `sourceSnapshotDigest` ou `preflightDigest`. As projeções JSON são as únicas representações consumidas semanticamente; qualquer divergência em relação aos documentos humanos bloqueia o fluxo. Um contrato de entrega declara mecanicamente a constituição como instrução de sistema, a arquitetura como política confiável, a intenção como dado do usuário, os sinais mecânicos da intenção como obrigações de revisão, o workspace como evidência não confiável e o schema como saída estruturada estrita. A saída da IA é obrigada a repetir o `contextDigest` calculado sobre esse pacote e eventual revisão humana, além de seguir o documento completo de `aegis.semantic_draft.v3`. Respostas de outro contexto são rejeitadas.

Antes da deliberação, um detector local procura apenas sinais universais e conservadores: expressões vazias, comparadores sem operando e alegações de qualidade como desempenho, latência, frequência ou ausência de alocações. Ele não interpreta a fórmula nem escolhe valores. Expressões incompletas precisam ser ligadas a uma lacuna material e decisão; restrições de qualidade precisam virar requisito `QUALITY` com método, métrica, alvo quantificado e condições de medição. Adjetivos sem alvo também exigem decisão. Todos os sinais são preservados no contrato e adulterações são rejeitadas.
...
O escopo descreve comportamento, não topologia interna. Caminhos de `src/` aparecem como evidência observada ou, quando o usuário exigir uma exportação pública específica, dentro de um requisito com proveniência. Nomes de arquivos, classes, buffers ou estruturas inventados pela IA são rejeitados; detalhes técnicos nomeados só podem virar obrigação quando constarem da intenção, de decisão humana ou de norma confiável aplicável.
