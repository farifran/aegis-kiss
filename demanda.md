...
Cada dimensão de determinismo termina em `SPECIFIED`, `NOT_APPLICABLE`, `DECISION_REQUIRED` ou `GAP_FOUND`. O Harness, não a IA, calcula a autoridade de fechamento. Texto do modelo sozinho não pode certificar uma regra nem sua inaplicabilidade: `SPECIFIED` exige regra autorizada e caso exato; `NOT_APPLICABLE` exige prova contrafactual autorizada contra o testemunho da ficha. `DECISION_REQUIRED` fica provisório até escolha humana. `GAP_FOUND` bloqueia a criação do Wizard. O contrato expõe ainda `effectiveDeterminismStatus`, recalculado depois das resoluções humanas, para impedir que uma decisão pendente pareça concluída.
