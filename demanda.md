...
Não são enviados à IA metadados da captura, contadores, tamanhos, hashes de arquivos, termos sem correspondência, `sourceSnapshotDigest` ou `preflightDigest`. As projeções JSON são as únicas representações consumidas semanticamente; qualquer divergência em relação aos documentos humanos bloqueia o fluxo. Um contrato de entrega declara mecanicamente a constituição como instrução de sistema, a arquitetura como política confiável, a intenção como dado do usuário, o workspace como evidência não confiável e o schema como saída estruturada estrita. A saída da IA é obrigada a repetir o `contextDigest` calculado sobre esse pacote e eventual revisão humana, além de seguir o documento completo de `aegis.semantic_draft.v1`. Respostas de outro contexto são rejeitadas.

O rascunho declara explicitamente sua interpretação, os contextos arquiteturais detectados, revisão de complexidade e revisão de riscos. A aplicabilidade de cada regra é calculada localmente a partir desses contextos. Sobre-engenharia detectada deve produzir alternativas mais simples; segurança, confiabilidade, privacidade, desempenho, integridade e complexidade são sempre consideradas, e achados recebem nível, mitigação e requisitos afetados. Conflitos distinguem o pedido original da solução recomendada: podem chegar ao Wizard vinculados a uma decisão, mas a recomendação precisa ser compatível ou apoiada por emenda aprovada. Toda decisão referencia requisitos, invariantes ou riscos que precisam ser recompilados caso outra opção seja escolhida.
