...
Uma revisão adversarial curta confronta a própria solução recomendada antes da entrega. Ela registra no máximo três objeções materiais e aponta para os requisitos, invariantes, riscos ou decisões que já absorveram cada consequência. Demandas com decisão, conflito de política, simplificação, risco alto, possível violação literal ou regra marcada para revisão adversarial não podem declarar parecer vazio; o achado precisa citar a regra que o tornou obrigatório. O parecer não é uma segunda especificação nem outro agente: no fluxo atual ele integra a mesma deliberação estruturada; uma segunda chamada independente só será adicionada junto do adaptador de IA e apenas para demandas materiais.

Cada requisito contém um caso `HAPPY_PATH` e pelo menos um caso `FAILURE` ou `BOUNDARY`. Cada decisão precisa ligar sua resposta recomendada a pelo menos um desses casos por `decisionBinding`; sem efeito contratual demonstrável, a assinatura falha com `DECISION_EFFECT_NOT_MATERIALIZED`. Esses casos especificam provas, mas não apontam para executores nem autorizam scripts. Uma escolha diferente da recomendada retorna `SEMANTIC_RECOMPILATION_REQUIRED`; a resolução humana é incorporada à próxima requisição e requisitos, riscos e casos devem ser recompilados antes da assinatura.

No `aegis.issue_contract.v8`, a recomendação permanece somente dentro da decisão que a explica, sem cópia redundante. `humanResolutions` preserva pergunta, resposta ou correção, rótulo, justificativa, método e digest do rascunho. `approval` permanece `null` no rascunho e só registra a declaração explícita e o digest exato aprovado no contrato final. O pedido de confirmação também mostra os casos de aceitação afetados. Assim, recomendação ausente de resposta nunca é tratada como decisão humana.
...
- [ ] **Adaptador de IA Ponta a Ponta:** Um provedor externo consome a interface semântica e devolve `aegis.semantic_draft.v4`.
