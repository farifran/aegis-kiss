...
Não são enviados à IA metadados da captura, contadores, tamanhos, hashes de arquivos, termos sem correspondência, `sourceSnapshotDigest` ou `preflightDigest`. As projeções JSON são as únicas representações consumidas semanticamente; qualquer divergência em relação aos documentos humanos bloqueia o fluxo. Um contrato de entrega declara mecanicamente a constituição como instrução de sistema, a arquitetura como política confiável, a intenção como dado do usuário, os sinais mecânicos da intenção como obrigações de revisão, o workspace como evidência não confiável e o schema como saída estruturada estrita. O Harness cria `aegis.semantic_request.v9` e sua ficha `aegis.semantic_worksheet.v1`; a IA devolve somente `aegis.semantic_opinion.v2`, repetindo o `worksheetDigest` recebido. O Harness valida esse vínculo e compila IDs, referências, estados, digests e o contrato final. Respostas de outro contexto são rejeitadas.

Antes da deliberação, um detector local procura apenas sinais universais e conservadores: expressões vazias, comparadores sem operando, alegações de qualidade, promessas de determinismo e representações públicas com largura explícita em bits. Ele não interpreta fórmulas, escolhe valores nem decide políticas de overflow. Expressões incompletas precisam ser ligadas a lacuna material e decisão. Uma qualidade só vira requisito `QUALITY` quando já existe alvo autorizado, com método, métrica, procedência, estado de evidência e condições; adjetivos como “rápido”, “alta frequência” ou “Zero-GC friendly” permanecem metas em `nonNormativeItems`. O mesmo campo preserva opções e exemplos — algo introduzido por “como”, por exemplo — sem promovê-los a obrigação. `MODEL_PROPOSAL` deixou de ser fonte válida de alvo, requisito ou pergunta. Uma saída de largura fixa pode ser provada por `EXACT_WIDTH` sem inventar políticas laterais; quantidades que podem escapar da faixa exigem comportamento e casos `BOUNDARY` estruturados. Assim, layout e largura são fatos mecânicos, enquanto domínio e overflow continuam sendo semântica explícita. Promessas de determinismo ativam onze dimensões independentes, incluindo `ROUNDING` e `REMAINDER_DISTRIBUTION` separadamente. A ficha gera um testemunho de contraexemplo tipado para cada dimensão; o parecer liga cada ocorrência a um `subject` observável, podendo repetir a mesma dimensão para campos ou resultados distintos. Toda representação pública limitada recebe sua própria revisão de `BOUNDED_ARITHMETIC`. Todos os sinais e testemunhos são preservados no contrato e adulterações são rejeitadas.

O rascunho declara explicitamente sua interpretação, os contextos arquiteturais detectados, revisão de complexidade e revisão de riscos. A aplicabilidade de cada regra é calculada localmente a partir desses contextos e de uma lista curta de referências literais; a IA devolve somente avaliações aplicáveis. Os sinais encontrados são injetados mecanicamente na requisição e no contrato, sem transformar correspondência lexical em veredito semântico. Sobre-engenharia detectada deve produzir alternativas mais simples. Segurança, confiabilidade, privacidade, desempenho, integridade e complexidade são sempre consideradas, sem um checklist declarativo redundante; achados recebem nível, mitigação e requisitos afetados. Conflitos distinguem o pedido original da solução recomendada. Regras `hard` e complexidade meramente sugerida são corrigidas pelo tribunal e não chegam ao Wizard. Uma regra `default` só pode gerar pergunta quando uma exigência observável e explícita ainda deixar uma decisão material. Toda decisão precisa corresponder a uma lacuna material e referenciar requisitos, invariantes ou riscos que serão recompilados caso outra opção seja escolhida.
...
O escopo descreve comportamento, não topologia interna. Caminhos citados recebem um papel explícito: `PUBLIC_SURFACE`, `IMPLEMENTATION_CONSTRAINT`, `IMPLEMENTATION_SUGGESTION` ou `EVIDENCE_ONLY`. Superfície pública não obriga implementação monolítica; sugestão e evidência não podem aparecer como obrigação. Nomes de classes, buffers ou estruturas inventados pela IA são rejeitados; detalhes técnicos só podem virar obrigação quando constarem da intenção, de decisão humana ou de norma confiável aplicável.

Contextos arquiteturais, requisitos, riscos, lacunas e simplificações carregam uma `basis` mínima que distingue intenção do usuário, decisão humana, Constituição, política arquitetural, evidência observada em `src/` e análise própria do modelo. Trechos da intenção precisam existir literalmente, decisões precisam ter sido resolvidas por uma pessoa e referências ao workspace precisam indicar uma linha realmente enviada à IA. Todo requisito exige ao menos uma fonte normativa; análise do modelo ou código observado, isoladamente, não criam obrigação. Além disso, `intentClaims` classifica cada afirmação material como obrigação, proibição, meta, opção, exemplo ou ambiguidade e a liga à sua destinação. Isso torna visível quando uma sugestão foi indevidamente promovida ou uma exigência foi omitida.

Uma revisão adversarial residual confronta a solução depois que os detectores e a política arquitetural já fizeram seu trabalho. Ela procura contradições, deriva semântica, omissões, lacunas de determinismo, perguntas artificiais e riscos técnicos; cada achado declara sua categoria, destinação e os elementos que incorporaram a correção. Não existe teto arbitrário de três achados, e o parecer não precisa repetir sinais ou violações já tratados em `policyAssessments`. No fluxo atual ele integra a mesma deliberação estruturada; uma segunda chamada independente só será adicionada junto do adaptador de IA e apenas para demandas materiais.

Cada par dimensão/`subject` termina em `SPECIFIED`, `NOT_APPLICABLE`, `DECISION_REQUIRED` ou `GAP_FOUND`. O Harness, não a IA, calcula a autoridade de fechamento. Texto do modelo sozinho não pode certificar uma regra nem sua inaplicabilidade. `SPECIFIED` passa por três portões mecânicos: a dimensão precisa ser aplicável ao `subject`, o caso precisa cobrir literalmente os dois lados do witness e a resolução tipada precisa pertencer à dimensão e declarar resultados observáveis exatos. Assim, BigInt não prova ordenação, conservação não prova canonicalização e largura de representação não prova overflow. `NOT_APPLICABLE` exige uma regra estrutural tipada, a classe de entrada ausente e evidência explícita sobre a própria dimensão; texto apenas relacionado não basta. `DECISION_REQUIRED` fica provisório até escolha humana. `GAP_FOUND` bloqueia a criação do Wizard. O witness descreve uma prova metamórfica futura, mas o preflight não executa nem cria produto ou testes. O contrato expõe ainda `effectiveDeterminismStatus`, recalculado sobre todas as decisões humanas pendentes: `SEMANTICALLY_CLOSED` significa que o contrato está semanticamente fechado, não que uma implementação já foi verificada.

Cada requisito contém um caso `HAPPY_PATH` e pelo menos um caso `FAILURE` ou `BOUNDARY`. Cada resposta do Wizard declara um `contractEffect` distinto; o efeito recomendado deve ser literalmente igual ao resultado de pelo menos um caso ligado por `decisionBinding`. Uma escolha diferente retorna `SEMANTIC_RECOMPILATION_REQUIRED`, e o novo contrato só é aceito quando o efeito escolhido aparece na especificação normativa. Casos de limite usam também `boundaryBinding`, exclusivo, indicando lado, comportamento e valor esperado; um caso genérico não pode mais fingir que prova várias regras diferentes. Esses casos não apontam para executores nem autorizam scripts.

No `aegis.issue_contract.v13`, a recomendação permanece somente dentro da decisão que a explica, sem cópia redundante. `humanResolutions` preserva pergunta, resposta ou correção, rótulo, justificativa, efeito contratual, método e digest do rascunho. `approval` permanece `null` no rascunho e só registra a declaração explícita e o digest exato aprovado no contrato final. O pedido de confirmação mostra os efeitos e casos de aceitação afetados. Assim, recomendação ausente de resposta nunca é tratada como decisão humana.
...
- [ ] **Adaptador de IA Ponta a Ponta:** Um provedor externo consome `aegis.semantic_request.v9` e devolve somente `aegis.semantic_opinion.v2`.
