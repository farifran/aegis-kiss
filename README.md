...
**Demand anchors:** runtime projects mechanical anchors (`aegis_materialize_demand_anchors_json`) into raw/aider prompts (human lines: SEED/TOKENS/SEARCH/… + json), capability `runtime.demand_anchors`, and handover. Structured demands add `goal` / `targets_header` / `done_when`. Evidence rank: layer0 → seed → anchors → reads → search/git/tools. **Forensics exit gate:** single seed/named alvo; rewrite hallucinated reasons. **Repair** gets an explicit FORENSICS HANDOFF block. **Discovery** scrubs false “operator named …” prose when no operator path exists. **Repair preflight** warns if dense tokens never appear in added lines (`AEGIS_DEMAND_TOKEN_PREFLIGHT=hard` to fail). **Adversarial** is tools-first (no search_symbol). **Validation** gets a TRIBUNAL SUMMARY block.
