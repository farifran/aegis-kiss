...
export class ClearingHouse {
  ...
  private _isOrderAdmitted(ord: TransferOrder, now: bigint, batchQuarantined: Set<string>): boolean {
    ...
    if (sender.bucket.peekTokens(now) < requiredBits) {
  ...
  private _admitOrders(
    ...
    for (const ord of orders) {
      if (this._isOrderAdmitted(ord, now, batchQuarantined)) {
        admitted.push(ord);
      } else {
        rejectedCount++;
      }
  ...
  private _commitBatch(
    ...
    for (const ord of orders) {
      ...
      if (sender) {
        const requiredBits = (ord.amount + ord.fee) * 8n;
        sender.bucket.consume(requiredBits, now);
      }
    }
  ...
  processBatch(orders: readonly TransferOrder[], nowMs?: bigint): BatchResult {
    ...
    this._globalChecksum = (this._globalChecksum ^ merkle ^ this._treasuryBalance) & 0xFFFFFFFFFFFFFFFFn;
