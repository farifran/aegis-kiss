export class TokenBucket {
  ...
  constructor(maxBytes: bigint, mbps: number) {
    ...
    if (!Number.isFinite(mbps) || mbps < 0 || mbps * 8000 > Number.MAX_SAFE_INTEGER || (mbps > 0 && Math.round(mbps * 8000) === 0)) throw new RangeError("mbps must be a non-negative safe number with representable rate");
