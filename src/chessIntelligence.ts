...
export class ChessIntelligence {
  ...
  constructor() {
    ...
    for (let i = 0; i < 12 * 64 + 1 + 16 + 65; i++) { state = (state * 6364136223846793005n + 1442695040888963407n) & 0xFFFFFFFFFFFFFFFFn; table.push(state); }
  ...
  _generateWhiteMoves(): ChessMove[] {
    ...
    if (oneStep >= 56) { moves.push({ from, to: oneStep, promotion: 4 }); moves.push({ from, to: oneStep, promotion: 1 }); }
    else { moves.push({ from, to: oneStep }); if (from >= 8 && from <= 15 && (allOcc & (1n << BigInt(from + 16))) === 0n) moves.push({ from, to: from + 16 }); }
    }
    const leftCap = from + 7; if (from % 8 > 0 && leftCap < 64) { if ((enemyOcc & (1n << BigInt(leftCap))) !== 0n) { if (leftCap >= 56) moves.push({ from, to: leftCap, promotion: 4 }); else moves.push({ from, to: leftCap }); } else if (leftCap === this._enPassantSquare) moves.push({ from, to: leftCap, isEnPassant: true }); }
    const rightCap = from + 9; if (from % 8 < 7 && rightCap < 64) { if ((enemyOcc & (1n << BigInt(rightCap))) !== 0n) { if (rightCap >= 56) moves.push({ from, to: rightCap, promotion: 4 }); else moves.push({ from, to: rightCap }); } else if (rightCap === this._enPassantSquare) moves.push({ from, to: rightCap, isEnPassant: true }); }
    }
    ...
    if ((this._castlingRights & 1) !== 0 && (allOcc & 0x60n) === 0n && !this.isSquareAttacked(4, 1) && !this.isSquareAttacked(5, 1) && !this.isSquareAttacked(6, 1)) moves.push({ from: 4, to: 6, isCastling: true });
    if ((this._castlingRights & 2) !== 0 && (allOcc & 0x0En) === 0n && !this.isSquareAttacked(4, 1) && !this.isSquareAttacked(3, 1) && !this.isSquareAttacked(2, 1)) moves.push({ from: 4, to: 2, isCastling: true });
    return moves;
  }
  ...
  _generateBlackMoves(): ChessMove[] {
    ...
    if (oneStep <= 7) { moves.push({ from, to: oneStep, promotion: 10 }); moves.push({ from, to: oneStep, promotion: 7 }); }
    else { moves.push({ from, to: oneStep }); if (from >= 48 && from <= 55 && (allOcc & (1n << BigInt(from - 16))) === 0n) moves.push({ from, to: from - 16 }); }
    }
    const leftCap = from - 9; if (from % 8 > 0 && leftCap >= 0) { if ((enemyOcc & (1n << BigInt(leftCap))) !== 0n) { if (leftCap <= 7) moves.push({ from, to: leftCap, promotion: 10 }); else moves.push({ from, to: leftCap }); } else if (leftCap === this._enPassantSquare) moves.push({ from, to: leftCap, isEnPassant: true }); }
    const rightCap = from - 7; if (from % 8 < 7 && rightCap >= 0) { if ((enemyOcc & (1n << BigInt(rightCap))) !== 0n) { if (rightCap <= 7) moves.push({ from, to: rightCap, promotion: 10 }); else moves.push({ from, to: rightCap }); } else if (rightCap === this._enPassantSquare) moves.push({ from, to: rightCap, isEnPassant: true }); }
    }
    ...
    if ((this._castlingRights & 4) !== 0 && (allOcc & 0x6000000000000000n) === 0n && !this.isSquareAttacked(60, 0) && !this.isSquareAttacked(61, 0) && !this.isSquareAttacked(62, 0)) moves.push({ from: 60, to: 62, isCastling: true });
    if ((this._castlingRights & 8) !== 0 && (allOcc & 0x0E00000000000000n) === 0n && !this.isSquareAttacked(60, 0) && !this.isSquareAttacked(59, 0) && !this.isSquareAttacked(58, 0)) moves.push({ from: 60, to: 58, isCastling: true });
    return moves;
  }
  ...
  isThreefoldRepetition(): boolean {
    let count = 0;
    for (const h of this._history) { if (h === this._zobristHash) count++; }
    return count >= 2;
  }
  ...
  evaluatePosition(): number {
    ...
    while (bb > 0n) { whiteScore += val; bb = bb & (bb - 1n); }
    }
    for (let p = 6; p < 12; p++) {
    let bb = this._pieces[p] ?? 0n;
    const val = pieceValues[p] ?? 0;
    while (bb > 0n) { blackScore += val; bb = bb & (bb - 1n); }
    }
