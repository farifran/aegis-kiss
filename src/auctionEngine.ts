...
export class AuctionEngine {
  ...
  private static _computeMerkleRoot(leafHashes: bigint[]): bigint {
  ...
  private _buildEdgeMap(orders: readonly TransferIntent[], residualMap: Map<string, bigint>): Map<string, Map<string, bigint>> {
  ...
  private _deductEdgeOrders(
    from: string,
    to: string,
    amountToDeduct: bigint,
    ctx: { orders: readonly TransferIntent[]; residualMap: Map<string, bigint> }
  ): void {
  ...
  private _resolveSingleCycle(a: string, b: string, c: string, edgeMap: Map<string, Map<string, bigint>>): bigint {
  ...
  private _resolveDirectCycles(
    edgeMap: Map<string, Map<string, bigint>>,
    orders: readonly TransferIntent[],
    residualMap: Map<string, bigint>,
  ...
  private _executePartialFill(
    ...
    residualMap: Map<string, bigint>,
    ctx: { participants: Set<string>; leafHashes: bigint[] }
  ): bigint {
    let totalResidualDemand = 0n;
    for (let i = 0; i < orders.length; i++) {
      const ord = orders[i];
      if (ord !== undefined) totalResidualDemand += residualMap.get(ord.id) ?? 0n;
    }
