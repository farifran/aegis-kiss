...
export class AuctionEngine {
  ...
  _computeMerkleRoot(leaves: readonly bigint[]): bigint {
  ...
  _matchBilateralLeg(ordA: TransferIntent, ordB: TransferIntent, resA: bigint, resB: bigint): bigint {
    if (ordA.from !== ordB.to || ordA.to !== ordB.from) return 0n;
    return resA < resB ? resA : resB;
  }

  _resolveBilateralCycles(orders: readonly TransferIntent[], residuals: BigInt64Array, leafHashes: bigint[], pMap: Map<string, boolean>): bigint {
    let cycleVolume = 0n;
    ...
    leafHashes.push(this._fnv1a64('2cycle:' + ordA.id + '->' + ordB.id + ':' + minFlow.toString()));
    }
    }
    }
    return cycleVolume;
  ...
  _checkTriangularFlow(ordA: TransferIntent, ordB: TransferIntent, ordC: TransferIntent, res: { resA: bigint; resB: bigint; resC: bigint }): bigint {
  ...
  _checkTriangleAtJK(idx: [number, number, number], orders: readonly TransferIntent[], ctx: { residuals: BigInt64Array; leafHashes: bigint[]; pMap: Map<string, boolean> }): bigint {
    const i = idx[0];
    const j = idx[1];
    const k = idx[2];
    const ordA = orders[i];
  ...
  _searchTriangleK(i: number, j: number, orders: readonly TransferIntent[], ctx: { residuals: BigInt64Array; leafHashes: bigint[]; pMap: Map<string, boolean> }): bigint {
    const n = orders.length;
    let volume = 0n;
    for (let k = 0; k < n; k++) {
    if (k === i || k === j) continue;
  ...
  _searchTriangularRing(i: number, orders: readonly TransferIntent[], ctx: { residuals: BigInt64Array; leafHashes: bigint[]; pMap: Map<string, boolean> }): bigint {
    const ordA = orders[i];
    const resA = ctx.residuals[i] ?? 0n;
  ...
  _resolveTriangularCycles(orders: readonly TransferIntent[], residuals: BigInt64Array, leafHashes: bigint[], pMap: Map<string, boolean>): bigint {
    let cycleVolume = 0n;
  ...
  _executePartialFill(orders: readonly TransferIntent[], matchedVolume: bigint, residuals: BigInt64Array, ctx: { leafHashes: bigint[]; pMap: Map<string, boolean> }): bigint {
    ...
    ctx.leafHashes.push(this._fnv1a64('partial:' + ord.id + ':' + ord.from + '->' + ord.to + ':' + fill.toString()));
    }
    }
