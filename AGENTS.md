...
- Todo valor público de representação finita exige revisão explícita. Diferencie `REPRESENTATION_WIDTH`, que prova apenas formato e largura, de `ENCODED_VALUE`, que exige domínio, capacidade e comportamento `BOUNDARY` exclusivo para cada lado. Wrap ou truncamento silencioso são proibidos salvo exigência humana textual.
...
- Promessas de determinismo revisam explicitamente todas as dimensões universais do protocolo, classificando cada uma como especificada, não aplicável ou dependente de decisão.
- Toda classificação de determinismo declara sua origem. `NOT_APPLICABLE` exige prova contrafactual de que variar somente aquela dimensão preserva exatamente o mesmo resultado público permitido; Constituição genérica ou análise do modelo, isoladamente, não provam inaplicabilidade. Um default mecânico só pode ser aplicado quando citar uma regra arquitetural confiável que o autorize.
