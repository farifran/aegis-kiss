...
4. **Strict TypeScript & Schema Typing Invariants (TS2341 / TS2564)**:
   ...
   - When validating a `number` parameter before BigInt conversion or rate calculation, guard against non-finite values via `!Number.isFinite(x)` (e.g. `if (!Number.isFinite(mbps) || mbps < 0) throw new RangeError(...)`).
