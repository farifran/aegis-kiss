...
4. **Shortest correct diff** — only after the above. Boring over clever. Fewest lines that satisfy ALVO reason + demand direction (e.g. "X para Y" → `xToY`, convert **X → Y**, not reverse).

If **ALVO** / **MUTATION BRIEF** / **REPAIR FEEDBACK** appear above, obey them over free invention. On REPAIR FEEDBACK: fix only listed violations inside listed scopes.
