...
raw_mode_minimal_artifact_instructions() {
  case "${AEGIS_MODE}" in
    ...
    discovery)
      ...
      ;;
    optimize)
      printf '%s' "MINIMAL OPTIMIZE (CODE ELEGANCE & DESIGN, advise only): emit ONLY status+basis+improvements. status=no_improvement_needed|can_improve. Evaluate against 4 Pillars of Elegance: (1) single source of truth (replace redundant mutable state with pure getters), (2) flat control flow (early guard clauses), (3) semantic minimalism & O(1), (4) zero type casts ('as'). can_improve with 1 sharp item: target_files exact from MUTATION RESULT files_changed, change=imperative surgical edit, why_safe=why behavior unchanged. No edits, no diff. Full contract: skill file."
      ;;
    adversarial)
