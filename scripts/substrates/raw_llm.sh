...
assemble_system_prompt() {
  ...
  if [[ "${AEGIS_MODE}" == "forensics" ]]; then
    ...
    mode_specific_instructions="MINIMAL ADVERSARIAL ARTIFACT: emit ONLY {\"status\": \"challenged|verified\", \"findings\": [{\"type\": \"...\", \"severity\": \"...\", \"description\": \"...\", \"supported_by_evidence\": true|false, \"evidence_refs\": [\"...\"]}]}. Set status to 'challenged' if there are logical/mathematical bugs or boundary violations in the candidate diff, otherwise 'verified'. Be mathematically precise: do NOT generate false findings for mathematically equivalent expressions (e.g. 1024 * 1024 is identical to 1024^2 or 1048576). The runtime injects mode, candidate_result, top-level evidence_refs, and handover_attention — do NOT emit them."
  elif [[ "${AEGIS_MODE}" == "validation" ]]; then
    mode_specific_instructions="MINIMAL VALIDATION ARTIFACT: emit ONLY {\"verdict\": \"accepted|rejected\", \"basis\": \"...\"}. Reject the candidate ONLY if there is a real, functional logic bug or syntax error. If the adversarial findings are false positives or mathematically equivalent (e.g. 1024*1024 vs 1024^2), ignore them and set verdict to 'accepted'. The runtime injects mode, validated_candidate, findings, evidence_refs, and handover_attention — do NOT emit them."
  fi
