...
| **Supervisor Briefing (In-Memory & Sovereign Compiler)** | `aegis_briefing_expand_json` & `aegis_briefing_typecheck_json` (`briefing.sh`): 100% in-memory curl streaming (zero `/tmp/` I/O). Strict JSON envelope validation + sovereign `tsc --noEmit` compiler gate with sanitized diagnostic feedback injected directly into self-healing retry loop. Universal handling of in-place barrel modifications (`barrelFrom: null`). |
