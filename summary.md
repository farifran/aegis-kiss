...
Current Suite Status: **all 18 npm-chained test suites passing green (Exit 0)** (`npm run aegis:test`), with static verification, eslint boundaries enforcement, type checking, and cross-platform verification (validated on Linux and macOS). The chain includes `aegis:test:required-evidence-augmentation`, `aegis:test:sovereignty-fallback`, the fail-powered `aegis:test:model-boundary-idempotency` (proves an injected model survives a stripped `env -i` config re-source and can never be clobbered back to a default), `aegis:test:model-boundary-idempotency-cache` (proves KV-cache salt derivation is idempotent and rotates on any physical surface mutation, and that the prompt prefix is byte-stable across same-mode runs), and the fail-powered `aegis:test:net-new-index-isolation` (proves a net-new file creation through the aider substrate leaves the operator's main git index pristine — reproduced against a production-shaped linked worktree).
...
| `scripts/lib/common.sh` | Source-only shared library: tagged logging (`aegis_log`/`aegis_warn`/`aegis_fatal` via `AEGIS_LOG_TAG`), the portable `measure` timing helper (`$(date +%s)` subshells, Bash 3.2-safe), and `derive_cache_salt` — a pure SHA256 over surface HEAD + uncommitted-diff/untracked digest + handover `generated_at`, used for optional KV-cache partitioning. | Sourced by `runtime_aegis.sh`, `scripts/execute_mode.sh`, and both substrates. |
...
| `scripts/substrates/test/test_net_new_index_isolation.sh` | Fail-powered proof that net-new file creation through the aider substrate never pollutes the operator's main git index. Sets up a production-shaped **linked `git worktree`** surface off a main repo, drives the real substrate to create a net-new file (triggering the intent-to-add pre-materialization), and asserts the main repo's `git ls-files` and `git status` stay pristine. Chained as `aegis:test:net-new-index-isolation`. | Guards `scope_mutation_git_dir_to_surface` in `scripts/substrates/aider_substrate.sh`; complements `test_aider_substrate.sh` (whose standalone-repo surface did not reproduce the main-index leak). |
| `scripts/substrates/test/test_model_boundary_idempotency_cache.sh` | KV-cache boundary suite. Part 1 (fail-powered): `derive_cache_salt` is idempotent on an unchanged surface and MUST rotate on any physical mutation (tracked diff, net-new untracked file, or handover generation change) — red if a mutation fails to rotate the salt. Part 2: two consecutive same-mode runtime runs (via a request-capturing curl shim) must yield byte-identical system prompts and a byte-identical user stable prefix, with volatile identity confined to the tail and `cache_salt` present + stable. Chained as `aegis:test:model-boundary-idempotency-cache`. | Guards `derive_cache_salt` (`scripts/lib/common.sh`) and the prompt-topology inversion + `cache_salt` gating in `scripts/substrates/raw_llm.sh` / `scripts/execute_mode.sh`. |
...
* **Capacidade Generativa Expansiva (Criação de Arquivos *De Novo*)**: Mapeia intenção de criação diretamente em `.skills/discovery.md` e `.skills/forensics.md`.
  ...
  - *Autorização de Escopo*: A projeção `AEGIS_JQ_AUTHORIZED_TARGETS` (gate `forensics_repair_candidate_outside_discovery_scope` em `execute_mode.sh`) une vários canais do artefato promovido da Discovery: `observed_request_alignment.resolved_paths` **e `requested_paths`** (extração determinística de caminhos do investigation input pelo `structural.builder`), `operational_context.required_evidence` (prefixo `filesystem.read:` removido), `ranked_targets` de `explicit_request`, `next_attention_targets`, e os nós de topologia (boundaries/hotspots/entrypoints/bridges/surfaces). O canal `requested_paths` é crítico para **criação de arquivos net-new**: `resolved_paths` só contém arquivos existentes, portanto um arquivo inédito pedido explicitamente pelo operador só é autorizável por `requested_paths` — sem essa fonte, o gate rejeitava candidatos net-new (`forensics_repair_candidate_outside_discovery_scope`) a menos que o modelo os listasse em `required_evidence`. A autorização continua derivada exclusivamente do artefato promovido da Discovery, agora também pelo canal de extração-de-pedido, de forma determinística e independente do modelo.
...
### Post-v1.0.0 Hardening (runtime rails, repair feedback, KV-cache prefix stability)

* **Absolute worktree cleanup invariant** (`runtime_aegis.sh:cleanup_runtime`): the EXIT trap captures the terminating `$?` first. On **any non-zero exit** (fatal provider error, validation-rejection abort, unhandled failure under `set -Eeuo pipefail`) it force-expunges **every** disposable execution surface under the root — overriding the retention policy — so an operator workspace is never left with orphaned `git worktree` surfaces. On clean exit it still honors `AEGIS_RUNTIME_REMOVE_EXECUTION_SURFACE` but expunges **all** surfaces (not just the current mode's), closing a sibling-leak across multi-mode feedback pipelines. Observed live: abnormal-termination expunge fires on `exit_code=1`.
* **Structured repair feedback contract** (`scripts/execute_mode.sh` validation branch → `runtime_aegis.sh` repair precondition → `scripts/audit_epistemic_pipeline.sh`): on a `rejected` verdict, the validation normalizer projects the typed adversarial findings into a deterministic `operational_context.repair_feedback` object — `violations[]` (`origin`, `severity`, `target_files`, `structural_reason`, `evidence_refs`) built only from blocking findings (evidence-supported, severity ≥ medium, excluding `missing_evidence`) plus `authorized_scopes[]` (the candidate's changed files). The key is omitted for non-rejected verdicts. A feedback repair iteration hard-fails (`precondition_failed_structured_repair_feedback_missing_or_invalid`) unless this object is present and well-formed, forcing repair re-entry to consume deterministic parameters instead of free-form prose. The auditor type-checks `repair_feedback` when present.
* **Prompt topology inversion for KV-cache prefix stability** (`scripts/substrates/raw_llm.sh`): the raw-substrate prompt is stacked by decreasing token half-life — constitutional preamble + skill contract + stable manifest projection at the head (system prompt), and per-request volatile segments (investigation input, volatile manifest metadata `generated_at`/`execution_id`/`manifest_hash`, execution identity) pushed to the tail. The manifest is split into stable and volatile projections. This maximises prefix reuse for any serving-side prefix cache (and helps hosted endpoints' automatic prefix caching); it is always on and backend-agnostic. The aider mutation prompt keeps the actionable `mode_instructions` last (execution identity stays at the head) — moving it to the tail was found to degrade weak-model instruction following.
* **Optional `cache_salt` KV-cache partitioning** (`derive_cache_salt` in `scripts/lib/common.sh`; gated by `AEGIS_ENABLE_CACHE_SALT`, default `false`): when enabled, raw-substrate requests carry a `cache_salt` = SHA256(surface HEAD ∥ uncommitted-diff/untracked-file digest ∥ handover `generated_at`), so any physical mutation or handover promotion cryptographically rotates the cache partition and stale attention states become unreachable. This is a **vLLM-native** parameter (vLLM ≥ 0.8.3 / LMCache, `enable_blending:false`) and is only meaningful against a self-hosted vLLM+LMCache backend the operator controls; hosted OpenAI-compatible endpoints (e.g. NVIDIA NIM) ignore it, so it is disabled by default to avoid dead payload. LMCache itself is not part of this repository and requires a Linux + NVIDIA/CUDA host — it cannot run against a hosted cloud endpoint or on Apple Silicon. The feature-gate means the request stays byte-identical to the salt-less form when disabled.
...
- Disposable execution surfaces for mutation modes, with an absolute cleanup invariant that force-expunges all disposable surfaces on abnormal termination (verified live on `exit_code=1`).
- Bounded readonly cognition and bounded mutation surfaces.
- Automated metadata/envelope normalization in the Protocol VM.
- Deterministic structured repair feedback (`repair_feedback` with typed `violations[]` + `authorized_scopes[]`) enforced as a hard precondition for feedback repair re-entry.
- Prefix-stable prompt topology (volatile segments confined to the tail) plus optional, feature-gated `cache_salt` KV-cache partitioning for self-hosted vLLM/LMCache backends.
...
- ~~Decide whether `test_required_evidence_augmentation.sh` and `test_sovereignty_fallback.sh` should join the `aegis:test` npm chain~~ — **done**: both are chained (`aegis:test:required-evidence-augmentation`, `aegis:test:sovereignty-fallback`).
- `cache_salt` KV-cache partitioning is wired only on the raw substrate; the aider (mutation) path does not yet emit `cache_salt` (would need `extra_body` via aider model metadata) — relevant only if migrating to a self-hosted vLLM+LMCache backend.
