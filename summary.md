...
Current Suite Status: **all 18 npm-chained test suites passing green (Exit 0)** (`npm run aegis:test`), with static verification, eslint boundaries enforcement, type checking, and cross-platform verification (validated on Linux and macOS). The chain includes `aegis:test:required-evidence-augmentation`, `aegis:test:sovereignty-fallback`, the fail-powered `aegis:test:model-boundary-idempotency` (proves an injected model survives a stripped `env -i` config re-source and can never be clobbered back to a default), `aegis:test:model-boundary-idempotency-cache` (proves the prompt prefix is byte-stable across same-mode runs and that the `empty_mutation_candidate` gate forces rejection on blank/placeholder candidates), and the fail-powered `aegis:test:net-new-index-isolation` (proves a net-new file creation through the aider substrate leaves the operator's main git index pristine — reproduced against a production-shaped linked worktree).
...
| `scripts/lib/common.sh` | Source-only shared library: tagged logging (`aegis_log`/`aegis_warn`/`aegis_fatal` via `AEGIS_LOG_TAG`) and the portable `measure` timing helper (`$(date +%s)` subshells, Bash 3.2-safe). | Sourced by `runtime_aegis.sh`, `scripts/execute_mode.sh`, and both substrates. |
...
| `scripts/substrates/test/test_model_boundary_idempotency_cache.sh` | Prompt-stability suite. Part 1: exercises the REAL `empty_mutation_candidate` jq gate (extracted from `scripts/execute_mode.sh` source, not duplicated) — blank/placeholder diffs and empty `files_changed` force `rejected`, genuine candidates pass untouched. Part 2: two consecutive same-mode runtime runs (via a request-capturing curl shim) must yield byte-identical system prompts and a byte-identical user stable prefix, with volatile identity confined to the tail. Chained as `aegis:test:model-boundary-idempotency-cache`. | Guards the prompt-topology inversion in `scripts/substrates/raw_llm.sh` and the empty-candidate validation gate in `scripts/execute_mode.sh`. |
...
* **Reflexão Veloz (Aider lint gate)**: `aider_lint_gate.sh` — por arquivo, no loop de reflexão do Aider: sintaxe → `prettier --write` (se houver) → `eslint --fix` (se houver; residual error falha o gate) → `static_gate`. **Não** roda tsc de projeto nem suíte de testes.
* **Pré-voo pós-mutação (runtime evidence)**: `mutation_preflight.sh` — **uma vez** após o diff do Aider, no surface: `typescript.check` + `test.run` (capabilities reais), materializa payloads JSON e **hard-fail** da mutação se `status=failed`. Skip limpo sem tsconfig/tools. Desliga com `AEGIS_MUTATION_PREFLIGHT=0`.
...
- Prefix-stable prompt topology (volatile segments confined to the tail), stateless and backend-agnostic — no cache flags or partition keys inside the harness.
...
- ~~`cache_salt` KV-cache partitioning wiring~~ — **purged** (YAGNI audit): the speculative salt/valve infrastructure was removed entirely; prefix reuse now relies solely on the stateless prompt topology. Reintroduce only if a self-hosted vLLM+LMCache backend actually materializes.
