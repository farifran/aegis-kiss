...
Cada pergunta oferece 2–4 alternativas, uma recomendada, uma `resolutionClause` completa e apenas os `statePolicies` dos papéis ainda abertos que ela resolve. O padrão é **uma política por pergunta**. Não agrupe porque as políticas aparecem na mesma frase, pertencem ao mesmo módulo ou ambas usam tempo: isso é correlação de implementação, não acoplamento semântico. Agrupe duas ou mais políticas somente quando não existir nenhuma combinação mista válida entre suas alternativas; nesse caso, a evidência da pergunta deve explicar qual combinação seria inválida e por quê. Unidade, destino ou retenção de recurso; aptidão, unicidade ou isolamento de identidade; e fonte, igualdade, regressão ou replay temporal normalmente são decisões independentes e devem gerar perguntas distintas. A seleção posterior é mecânica; não espere outra chamada. Não comprima condições independentes de aceitação; cada condição explícita deve ter requisito e binding próprios. Defina a álgebra entre decisões e agregados, a fronteira de publicação atômica e a representação canônica quando aplicáveis. Use `forensic` e ao menos uma prova de cadência `forensic` quando atomicidade coexistir com recurso, tempo, identidade externa ou canonicalização. A primeira compilação de uma transição `forensic` deve sempre retornar `NEEDS_CONFIRMATION`: use as perguntas para resolver as escolhas de negócio ainda abertas. Essa confirmação humana e a revisão independente são obrigatórias antes de persistir o contrato.
