...
Edit only `src/index.ts`. Parent intent: # Issue: Implementação do EventEmitter & Barramento PubSub com Prioridades () ## Descrição Implementar uma classe EventEmitter fortemente tipada com suporte a listeners prioritários, emissão síncrona/assíncrona, execução
